...
namespace Ejer6_AndroidRelations
{
    class ListViewAdapter : BaseAdapter<Persona>
    {
        ...
        public override View GetView(int position, View convertView, ViewGroup parent)
        {
            ...
            if (row == null)
            {
                row = LayoutInflater.From(miContext).Inflate(Resource.Layout.lista, null, false);
            }
            TextView nombre = row.FindViewById<TextView>(Resource.Id.tvNombre);
            nombre.Text = miItems[position].nombre;
            TextView apellido = row.FindViewById<TextView>(Resource.Id.tvApellido);
            apellido.Text = miItems[position].apellido;
            TextView sexo = row.FindViewById<TextView>(Resource.Id.tvSexo);
            sexo.Text = miItems[position].sexo;
            TextView id = row.FindViewById<TextView>(Resource.Id.tvId);
